...
- `session-start` hook — the plugin's one `SessionStart` entry: runs every SessionStart hook and merges their statuses into one payload
...
| ---- | ----- | ----------- |
| [session-start](hooks/session-start.sh) | SessionStart | The plugin's only SessionStart entry. Runs the four hooks below in order and merges their statuses into one payload. A failed hook is reported as its own status line. Never blocks. |
