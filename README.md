...
| Teamwork | [agent-team-operation](rules/agent-team-operation.md) | Multi-agent team rounds: headroom-driven role rotation, YOLO workers within classified assignments, one writer per worktree, report files as the worker channel, persistent task acceptance ledger, internal review before the PR opens |
...
| [herdr-teamlead](skills/herdr-teamlead/SKILL.md) | Lead coding rounds in [Herdr](https://herdr.dev): measure headroom, assign roles, compose briefs, provision worktrees, and dispatch workers in verified YOLO mode. Track evidence-backed acceptance in a persistent task ledger independently of Herdr status. Model tiers use verified relaunches, qualification gates, and measured or unknown billing windows. A pinned judge rules on disputes and exhausted fix loops. |
