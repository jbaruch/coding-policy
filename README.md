...
- `session-start` hook — the plugin's one `SessionStart` entry, native for Claude Code and Codex: runs every SessionStart hook and merges their statuses into one payload
...
| ---- | ----- | ----------- |
| [session-start](hooks/session-start.sh) | SessionStart (native: Claude Code + Codex) | The plugin's only SessionStart entry, declared as a native hook so it keeps the session's environment. Runs the five hooks below in order and merges their statuses into one payload. A failed hook is reported as its own status line. Never blocks. |
...
| [check-tessl-latest](hooks/check-tessl-latest.sh) | SessionStart (via session-start) | Runs `tessl update --yes` and reports every `jbaruch/*` dependency's version, flagging one pinned instead of `latest` (`rules/dependency-management.md`). Third-party pins are out of scope. Never blocks. |
| [check-acr-latest](hooks/check-acr-latest.sh) | SessionStart (via session-start) | In a project with `agents.yaml`, runs `acr freshness run --policy install`: reconciles ACR dependencies declared `latest`, realizes changed files, and reports ACR's output (a `restart_required` notice included). Pinned tags and commits never move; `agents.yaml` is not rewritten. Silent without `agents.yaml` and in Herdr worker sessions. Never blocks. |
