...
| [install-reviewer](skills/install-reviewer/SKILL.md) | Enroll a consumer repo in the central fleet policy reviewer — scaffold the `.github/fleet-review-enabled` opt-in marker plus `.github/copilot-instructions.md`, then open a PR. The marker enrolls the repo in the `coding-policy-fleet-reviewer` GitHub App, which polls and reviews every marked repo against the `jbaruch/coding-policy` rules with the Codex CLI on a ChatGPT subscription (no API key). No per-repo workflow or secrets — the credential lives only in `coding-policy`. Supports `--override` for in-place upgrades. |
