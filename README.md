...
- `teamlead` skill + `agent-team-operation` rule + `herdr-team-status` hook — run a three-agent team round inside [Herdr](https://herdr.dev): measure each worker's subscription headroom, assign developer / tester / reviewer by measured headroom, clear each worker's context and send a fresh role brief, wait on the report file plus its `REPORT: ` marker, and hand the merge to `release`
...
| [teamlead](skills/teamlead/SKILL.md) | Run one task round as the lead of three coding agents in [Herdr](https://herdr.dev): roster the named workers, measure each one's subscription headroom, assign developer / tester / reviewer heaviest-role-to-most-headroom, write the role briefs from the packaged templates, clear each worker's context and dispatch, wait on the report file plus its `REPORT: ` marker (never on a single idle observation), then gate the round and hand the PR + merge to `release`. |
