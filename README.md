...
| [check-leftover-worktrees](hooks/check-leftover-worktrees.sh) | SessionStart | Reports worktrees holding uncommitted work on a branch carrying no commits of its own — the state `stop-handoff-hygiene` omits, that hook listing only trees safe to remove. Reads `skills/release/check-leftovers.sh` rather than reimplementing the predicate. Informative only, never blocks. |
