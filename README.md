...
| [herdr-teamlead](skills/herdr-teamlead/SKILL.md) | Lead coding rounds in [Herdr](https://herdr.dev): measure headroom, assign roles, compose briefs, provision worktrees, and dispatch workers in verified YOLO mode. Track evidence-backed acceptance in a persistent task ledger independently of Herdr status. Save retrospectives daily during active work and before worker transitions; retrieve them later without a live team. Model tiers use verified relaunches, qualification gates, and measured or unknown billing windows. A pinned judge rules on disputes and exhausted fix loops. |
