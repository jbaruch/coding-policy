"""Which supervision events need the lead, and which are noise.

Herdr records every change it observes in a worker, and the lead acknowledges
...
{"sha256": "5f084..."}}` has no meaning to read. Deciding whether the lead is
...
everything else wakes the lead, including a kind this module has never seen.
...
#: Every other kind wakes the lead, whether or not it appears here.
SUPPRESSIBLE = ("visible_observed", "recheck_due")

#: How many consecutive unchanged rechecks may be suppressed for one deferral
#: before the lead sees it anyway. A worker whose observed state never moves is
#: indistinguishable from a stalled one, and a stall is the lead's to judge.
...
def _verdict(event, member, state, quiet_rechecks):
    ...
    if seen >= MAX_QUIET_RECHECKS:
        return True, "state unchanged across {} rechecks: a stall is the lead's to judge".format(seen)
