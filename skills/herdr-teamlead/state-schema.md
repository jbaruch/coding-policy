...
A partitioned plan also carries `partition_proof`, copied unchanged from the
`validate-partition` result: `{"repo", "base", "head"}`, the full base and
head commits the slices were proven over (`head` null for a working-tree
proof). Writer: `plan --partition`. Reader: `verify-partition`, which refuses
a tip other than `head` and slices that do not cover exactly
`git diff base...head` (#460). A result without the proof is refused at
`plan`; re-validate with this build.
