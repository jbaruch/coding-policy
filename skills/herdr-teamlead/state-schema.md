...
| `diagnoses` | Unique `id`, `task`, `checkpoint`, `fix_round`, original `base_revision`, `remedy` (`continue`, `restructure` or `stop`), `bound` or null, `judge_agent`, `judge_evidence`, `scope`, `allowed_paths`, and the `plan` a bounded remedy authorized or null. A task's diagnoses move strictly down the remedy ladder and `stop` is terminal. |
