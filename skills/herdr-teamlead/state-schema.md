...
| `events` | Append-only `sequence`, `kind`, and structured `details` preserving approvals, waiting states, reservations, send transitions, results, transport retries, superseded review receipts, and recovery decisions. Kind `task_closed` carries `details.outcome` (`merged` or `abandoned`) and `details.evidence`; it ends the task's developer reservation until a later developer assignment reopens the task. Readers that predate the kind ignore it. |
