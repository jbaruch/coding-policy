...
| `close-task` | `task`, `outcome` (`merged` or `abandoned`), `evidence` | Append a `task_closed` event once the task merged or was abandoned, which releases its developer's reservation for `plan`. A later developer assignment reopens the task. Same closure replays; a different one is refused. |
