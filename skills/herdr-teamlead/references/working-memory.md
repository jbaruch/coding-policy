...
`reset_ready` is false if any required file has changed or become unavailable, or if a version-1 stow lists free-text gaps. A structured gap names its own recovery and does not block a reset. A true value covers only the saved local capture and its unchanged required files. It does not prove that the foreman captured every conversation fact, reconcile a fleet, satisfy the supervision gate, authorize interruption, or accept tasks. The foreman must still apply the separate handoff and supervision rules. New unresolved knowledge after the stow requires a new stow id.
...
Stow records have `kind: "stow"` and the stow input fields above, replacing `required_reads` with file receipts in the supplied order. Stow records are version 2; a version-1 stow keeps its free-text `gaps` and is never restamped. Captures and unresolved knowledge remain in the index even if those source files later disappear.
