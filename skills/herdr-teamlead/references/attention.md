...
A progress input contains exactly `id`, `task`, `summary`, `assessment`, and `sources`. Assessment is `verified`, `reported`, or `unknown`; at least one source must be the associated `task_ledger`. Use `reported` for unaccepted worker claims. The utility records its UTC event time. The catch-up labels each assessment **as recorded at that time**. Even `verified` means verified when recorded; revalidate the actual task ledger and source evidence before claiming current acceptance. This cached summary never replaces the task ledger or treats Herdr lifecycle labels as acceptance. Give a changed fact a new progress ID; preserve the earlier claim and correction.
