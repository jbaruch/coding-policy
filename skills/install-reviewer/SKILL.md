...
- Notes that each reviewer's verdict begins with a one-line load indicator (`"Policy loaded: N rule files from .tessl/tiles/jbaruch/coding-policy/rules/ (installed tile)."`), making it obvious when `tessl install` ran cleanly vs when the policy didn't reach the runtime
