...
- **Post-merge: published release (GitHub tag/asset)** — `skills/release/verify-github-release.sh <owner> <repo> <tag>` exits 0 iff the release exists at that exact tag, is published, and every asset is retrievable. The wrapper runs none of the three Tessl steps for this channel, and must NOT report the release confirmed on the run's conclusion alone.
