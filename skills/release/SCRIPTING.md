...
- **Non-empty review body halts unattended merge** — whether a `COMMENTED` body is substantive is a reasoning call the script can't make (`rules/script-delegation.md`). A non-empty `reviews.*.body` on a `COMMENTED` review must surface for an agent/human read, not auto-pass — a wrapper that merges on state alone reproduces the unread-body hole `rules/reviewer-feedback-reading.md` exists to close.
