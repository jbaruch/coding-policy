...
- **Post-merge verification** — three confirmations the script must perform, in order: (1) `main` actually advanced to the merge commit; (2) the publish/release workflow ran to **completion** — derive the merge commit SHA, resolve the publish run whose `headSha` equals that SHA, then `gh run watch <id> --exit-status` on **that exact run**. Never select by "latest on main" or any branch-only/limit-1 heuristic: `main` may advance again before the script starts watching, producing a false positive against the wrong run. Also never reduce this to "check it triggered" (fire-and-forget; scores zero against silent publish failures); (3) the new version is **live in the registry** — derive the version YOUR publish run produced from `tile.json#version` at the publish-run's head SHA (`git show <publish-run-head-sha>:tile.json | jq -r .version`), then compare against the registry's latest (Tessl: `tessl tile info <workspace>/<tile>`'s `Latest Version`; other registries: `npm view`, `pypi` JSON, etc.). The script must accept registry-latest >= your-version (newer is fine — means another merge interleaved and re-bumped) and fail loudly only on registry-latest-older or your-version-not-reachable. **Never** compare against "current latest equals expected" — that's the same race the publish-run lookup just fixed: in repos that auto-publish on every merge, another merge can advance the registry between your publish completing and your check running. The Tessl registry **never rejects a published version**: a failed moderation flag means the version still lands but won't surface in `tessl search`; a security finding means installs require an override flag — neither is rejection. Do not encode rejection or rejection-recovery as expected intermediate states or retry/wait conditions in the wrapper. Per `rules/ci-safety.md`'s extended Always Watch CI duty.
