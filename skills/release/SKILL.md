...
- Watch the publish workflow to a terminal state. Bind to the merge commit SHA + the `push` event, never to "latest on main": `merge_sha=$(git log -1 --format=%H)`, then resolve the run with `gh run list --branch main --workflow "<publish-workflow-name>" --json databaseId,headSha,event --jq '.[] | select(.headSha == "'"$merge_sha"'") | select(.event == "push") | .databaseId' | head -n 1`, then `gh run watch <run-id>`. Filtering by event excludes manual `workflow_dispatch` runs that may share the same SHA. The watch is a timing precondition for the registry check below, not the authoritative gate
