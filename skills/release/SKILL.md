...
- **Verify the new version is live in the registry** — for Tessl tiles, find the version YOUR publish run produced via `git show $(gh run view <publish-run-id> --json headSha --jq .headSha):tile.json | jq -r .version`, then compare against `tessl tile info <workspace>/<tile>`'s `Latest Version`. The registry's latest should equal your version OR be **newer** (newer means another merge landed after yours and re-bumped — not a failure; this repo auto-publishes on every merge, so interleaved merges are normal in busy periods). **Older** (or your version not reachable) after the publish run returned `success` is **CI failed** — diagnose the publish run and resolve the underlying failure. Naively re-running can create an extra release when the publish workflow includes a version-bump step (as this tile's `publish.yml` does via `tesslio/patch-version-publish`); the safer recovery is usually a follow-up commit that fires a fresh publish on merge. The Tessl registry **never rejects a published version**: a failed moderation flag (`moderationPassed: false`) means the version still lands and is fetchable but won't surface in `tessl search`; a security finding means `tessl install` requires an override flag to install — neither is rejection. Do not invent rejection as a failure mode. The Tessl registry **never rejects a published version**: a failed moderation flag (`moderationPassed: false`) means the version still lands and is fetchable but won't surface in `tessl search`; a security finding means `tessl install` requires an override flag to install — neither is rejection. Do not invent rejection as a failure mode; the real answer is always "the publish workflow failed, go look at the run"
