...
from foreman.composition import seat_holds
from foreman.errors import UsageError
from foreman.recovery import close_task, developer_reservations, register_task, task_closure, validate_store
from foreman.state import add_assignment, empty_state, load_state, save_state
...
class PlanHoldsTest(CliCase):
    ...
    def test_plan_skips_a_busy_worker(self):
        ...
        with patch("foreman.cli.supervision.load", return_value={"members": [member]}):
            code, out, err = self.plan()
        self.assertEqual(code, 0, err)
        result = json.loads(out)
        self.assertNotEqual(result["assignments"]["developer"], "grok")
