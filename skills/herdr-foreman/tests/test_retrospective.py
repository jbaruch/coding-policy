...
from foreman import retrospective as retro
...
## Improvements
The foreman will include the test command in the next brief; success is no setup retry.
"""
...
class RetrospectiveTest(unittest.TestCase):
    def setUp(self):
        temporary = tempfile.TemporaryDirectory(prefix="foreman-retrospective-")
    ...
    def test_sidecar_lock_serializes_aliases_and_releases_without_stale_flags(self):
        ...
        alias.symlink_to(self.root, target_is_directory=True)
        script = """import sys
from foreman import retrospective
