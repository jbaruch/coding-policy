#!/usr/bin/env bash
# Outcome-based tests for skills/herdr-foreman/provision-worktree.sh.
#
...
#
# Run: bash skills/herdr-foreman/tests/test_provision_worktree.sh
set -uo pipefail
...
main() {
  ...
  TMP="$(mktemp -d -t foreman-provision-test.XXXXXX)" || die "mktemp failed"
  trap cleanup EXIT
