#!/usr/bin/env bash
# Outcome-based tests for skills/herdr-foreman/foreman.sh.
#
...
#
# Covers:
#   1. Forwarding      -> `-m foreman` plus every argument, verbatim.
...
#
# Run: bash skills/herdr-foreman/tests/test_foreman_launcher.sh
set -uo pipefail
...
main() {
  SCRIPT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/foreman.sh"
  [[ -f "$SCRIPT" && -r "$SCRIPT" ]] || die "foreman.sh not found/readable at $SCRIPT"

  TMP="$(mktemp -d -t foreman-launcher-test.XXXXXX)" || die "mktemp failed"
  trap cleanup EXIT

  local skill="$TMP/skill"
  mkdir -p "$skill/foreman" || die "could not create $skill/foreman"
  cp "$SCRIPT" "$skill/foreman.sh" || die "could not copy the launcher into $skill"
  local fakepy="$TMP/fakepy"
  mk_fake_python "$fakepy"

  FAIL=0; PASS=0

  # 1. Arguments reach the module unchanged, behind `-m foreman`.
  local out rc
  out="$(env PY_BIN="$fakepy" bash "$skill/foreman.sh" plan --roles developer,tester,reviewer 2>&1)"; rc=$?
  if [[ $rc -eq 0 ]] && printf '%s' "$out" | grep -q -- "ARGV: -m foreman plan --roles developer,tester,reviewer"; then
    pass; else fail "forwarding: expected the full argv behind -m foreman, got rc=$rc out=$out"; fi

  # 2. The skill dir is prefixed onto PYTHONPATH, an existing value preserved.
  out="$(env PY_BIN="$fakepy" PYTHONPATH="/existing/path" bash "$skill/foreman.sh" state 2>&1)"; rc=$?
  ...
  cp "$SCRIPT" "$bare/foreman.sh" || die "could not copy the launcher into $bare"
  out="$(env PY_BIN="$fakepy" bash "$bare/foreman.sh" measure 2>&1)"; rc=$?
  ...
  out="$(env PY_BIN="$TMP/no-such-python" bash "$skill/foreman.sh" measure 2>&1)"; rc=$?
  ...
  out="$(env PY_BIN="$fakepy" FAKE_PY_VERSION=3.10 bash "$skill/foreman.sh" measure 2>&1)"; rc=$?
  ...
  out="$(env PY_BIN="$fakepy" FAKE_PY_VERSION_RC=7 bash "$skill/foreman.sh" measure 2>&1)"; rc=$?
  ...
  out="$(env PY_BIN="$fakepy" FAKE_PY_RC=7 bash "$skill/foreman.sh" measure 2>&1)"; rc=$?
  if [[ $rc -eq 7 ]]; then
  ...
  for legacy_version in 2.7 3.5; do
    out="$(env PY_BIN="$fakepy" FAKE_PY_VERSION="$legacy_version" FAKE_PY_LEGACY_GRAMMAR=1 \
      bash "$skill/foreman.sh" measure 2>&1)"; rc=$?
  ...
  out="$(env PY_BIN="$fakepy" FAKE_PY_VERSION=invalid bash "$skill/foreman.sh" measure 2>&1)"; rc=$?
