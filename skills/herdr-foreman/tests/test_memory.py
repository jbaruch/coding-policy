...
class MemoryTest(unittest.TestCase):
    def setUp(self):
        temporary = tempfile.TemporaryDirectory(prefix="foreman-memory-")
    ...
    def test_fresh_process_reads_saved_lesson_without_dispatch_state(self):
        ...
        result = subprocess.run(
            [_sys.executable, "-c", "import json,sys; from foreman.memory import list_lessons; print(json.dumps(list_lessons(sys.argv[1], sys.argv[2])))", str(self.state), LATER],
    ...
    def test_a_version_one_stow_migrates_its_gaps_to_operator_questions(self):
        self.stow()
        document = self.raw()
        text = "The current task ledger is unavailable; restore it before replacing the foreman."
    ...
    def test_atomic_failure_preserves_previous_history_and_retry_succeeds(self):
        self.record()
        before = memory.location(self.state).read_bytes()
        with patch("foreman.state.os.replace", side_effect=OSError("disk unavailable")):
    ...
    def test_live_owner_lock_refuses_concurrent_mutation(self):
        ...
        with state_lock(memory.location(self.state)):
            with self.assertRaisesRegex(StateError, "Another foreman command"):
                self.record(self.revision(), at=LATER)
    ...
    def test_cli_reports_dangling_index_without_replacing_it(self):
        ...
        for command in (["memory-list"], ["memory-record", "--record", str(draft)]):
            result = subprocess.run(
                [_sys.executable, "-m", "foreman", "--state", str(self.state), *command, "--now", LATER],
