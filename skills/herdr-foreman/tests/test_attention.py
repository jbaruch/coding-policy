...
from foreman.attention_view import catch_up
...
class AttentionTest(unittest.TestCase):
    def setUp(self):
        temporary = tempfile.TemporaryDirectory(prefix="foreman-attention-")
    ...
    def test_survives_independent_process_without_main_state_or_sources(self):
        ...
        script = "from foreman.attention_view import catch_up; import json, sys; print(json.dumps(catch_up(sys.argv[1], sys.argv[2])))"
    ...
    def test_lock_and_failed_atomic_write_preserve_history(self):
        ...
        self.assertEqual(self.saved(), before)
        with patch("foreman.state.os.replace", side_effect=PermissionError("fixture")):
            with self.assertRaises(StateError):
                self.record(obligation("another"))
        self.assertEqual(self.saved(), before)
