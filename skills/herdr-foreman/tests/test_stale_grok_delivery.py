...
from foreman import cli, recovery, report_delivery as delivery, state, supervision
...
class StaleGrokDeliveryTests(unittest.TestCase):
    ...
    def test_public_owner_recovery_replays_without_worker_input_or_byte_rewrites(self):
        ...
        del self.document['recovery']['approaches']
        ...
        self.assertEqual(json.loads(saved)['recovery']['schema_version'], recovery.RECOVERY_STORE_VERSION)
        ...
        for document, data in ((old_document, old_data), (self.document, self.data)):
            ...
            for index in (0, len(document['assignments']), -1, None, '0', 0.0, False, True, 1):
                ...
                for command in ('state', 'recover-report'):
                    args = ['bash', str(ROOT / 'foreman.sh'), command, '--state', str(ledger)]
    ...
    def test_an_image_first_turn_reads_its_own_prompt(self):
        ...
        with self.assertRaisesRegex(UsageError, 'grok_source_ambiguous'):
            self.recover()
        self.assertEqual(self.document, before)
        from foreman.report_delivery import source_prompt
    ...
    def test_migration_preserves_original_receipts_and_refuses_newer_embedded_records(self):
        ...
        del document['recovery']['approaches']
        before = copy.deepcopy(document)
        self.assertTrue(recovery.migrate_store(document['recovery']))
        self.assertEqual(document['recovery'], {**before['recovery'], 'schema_version': recovery.RECOVERY_STORE_VERSION, 'refusal_authorizations': [], 'diagnoses': [], 'legacy_ruling_recoveries': [], 'approaches': []})
