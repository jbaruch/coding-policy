#!/usr/bin/env bash
# Outcome-based tests for skills/herdr-foreman/label-workspaces.sh.
#
...
#   5. Caller excluded  -> the foreman's own pane is never relabelled as a worker.
...
#
# Run: bash skills/herdr-foreman/tests/test_label_workspaces.sh
set -uo pipefail
...
JSON
  ...
  # 5. The foreman's own pane is a caller, not a worker (w1:p1 is excluded).
  if ! printf '%s' "$ARGVTEXT" | grep -q "pane rename w1:p1"; then
    pass; else fail "caller: the foreman's pane must not be relabelled as a worker"; fi
