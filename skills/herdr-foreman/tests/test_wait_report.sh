#!/usr/bin/env bash
# Outcome-based tests for skills/herdr-foreman/wait-report.sh.
#
...
run() {
  ...
  OUT="$(env HERDR_ENV=1 HERDR_BIN="$FAKE" \
    FOREMAN_WAIT_INTERVAL_SEC=0 FOREMAN_WAIT_BUDGET_SEC=0 FOREMAN_BLOCKED_CONFIRM_SEC=0 \
    FOREMAN_REFUSAL_CONFIRM_SEC=0 \
...
main() {
  ...
  TMP="$(mktemp -d -t foreman-wait-test.XXXXXX)" || die "mktemp failed"
  trap cleanup EXIT
  ...
  RUN_SEQ=$((RUN_SEQ+1))
  OUT="$(env HERDR_ENV=1 HERDR_BIN="$FAKE" \
    FOREMAN_WAIT_INTERVAL_SEC=0 FOREMAN_WAIT_BUDGET_SEC=60 FOREMAN_BLOCKED_CONFIRM_SEC=0 \
...
${base}"
  ...
  #      and hand the pane to the foreman, not sit on the budget and not complete.
  RUN_SEQ=$((RUN_SEQ+1))
  local split_inside
  split_inside="  REPORT: /Users/jbaruch/.worktrees/round-3/reports/${base:0:3}
  ${base:3}"
  OUT="$(env HERDR_ENV=1 HERDR_BIN="$FAKE" \
    FOREMAN_WAIT_INTERVAL_SEC=0 FOREMAN_WAIT_BUDGET_SEC=600 FOREMAN_BLOCKED_CONFIRM_SEC=0 \
  ...
  # 21c. A bad FOREMAN_UNCONFIRMED_IDLE_READS is exit 2 with a named cause,
  #      never an arithmetic abort with no JSON.
  RUN_SEQ=$((RUN_SEQ+1))
  OUT="$(env HERDR_ENV=1 HERDR_BIN="$FAKE" FOREMAN_UNCONFIRMED_IDLE_READS=soon \
    bash "$SCRIPT" worker "$report" </dev/null 2>"$TMP/e21c")"; RC=$?
  if [[ $RC -eq 2 && -z "$OUT" ]] && grep -q "FOREMAN_UNCONFIRMED_IDLE_READS must be a positive integer" "$TMP/e21c"; then
  ...
  RUN_SEQ=$((RUN_SEQ+1))
  OUT="$(env HERDR_ENV=1 HERDR_BIN="$FAKE" FOREMAN_UNCONFIRMED_IDLE_READS=00 \
  ...
  RUN_SEQ=$((RUN_SEQ+1))
  OUT="$(env HERDR_ENV=1 HERDR_BIN="$FAKE" FOREMAN_UNCONFIRMED_IDLE_READS=02 \
    FOREMAN_WAIT_INTERVAL_SEC=0 FOREMAN_WAIT_BUDGET_SEC=600 FOREMAN_BLOCKED_CONFIRM_SEC=0 \
    FAKE_MARKER=timeout FAKE_STATUS=idle \
  ...
  RUN_SEQ=$((RUN_SEQ+1))
  OUT="$(env HERDR_ENV=1 HERDR_BIN="$FAKE" FOREMAN_WAIT_BUDGET_SEC=soon \
    bash "$SCRIPT" worker "$report" </dev/null 2>"$TMP/e21f")"; RC=$?
  if [[ $RC -eq 2 && -z "$OUT" ]] && grep -q "FOREMAN_WAIT_BUDGET_SEC must be a non-negative integer" "$TMP/e21f"; then
  ...
  run "$report" FAKE_MARKER=timeout FAKE_STATUS=idle FAKE_PANE_TEXT="$refusal" FOREMAN_UNCONFIRMED_IDLE_READS=1
  ...
  run "$missing" FAKE_MARKER=timeout FOREMAN_REFUSAL_CONFIRM_SEC=soon
  if [[ $RC -eq 2 && -z "$OUT" && "$ERRTEXT" == *FOREMAN_REFUSAL_CONFIRM_SEC* ]]; then
  ...
  run "$arriving" FAKE_MARKER=late FAKE_COUNTER="$TMP/arriving-marker" \
    ...
    FAKE_PANE_TEXT_AFTER="REPORT: $arriving" FAKE_APPEARING_REPORT="$arriving" FOREMAN_WAIT_BUDGET_SEC=600
  if [[ $RC -eq 0 ]] && printf '%s' "$OUT" | jq -e '.found == true' >/dev/null; then
  ...
  run "$missing" FAKE_MARKER=timeout FAKE_STATUS=working \
    ...
    FOREMAN_WAIT_BUDGET_SEC=600 FOREMAN_PROBE_TIMEOUT_MS=9999
  ...
  run "$report" FAKE_MARKER=timeout FAKE_STATUS=idle FAKE_GET_COUNTER="$TMP/once-idle-count" \
    PATH="$TMP/no-delay:$PATH" FOREMAN_WAIT_BUDGET_SEC=600
  ...
  RUN_SEQ=$((RUN_SEQ+1))
  OUT="$(env HERDR_ENV=1 HERDR_BIN="$FAKE" FOREMAN_WAIT_INTERVAL_SEC=0 FOREMAN_WAIT_BUDGET_SEC=0 \
  ...
  OUT="$(env HERDR_ENV=1 HERDR_BIN="$FAKE" FOREMAN_WAIT_INTERVAL_SEC=0 FOREMAN_WAIT_BUDGET_SEC=600 \
    FOREMAN_BLOCKED_CONFIRM_SEC=0 FOREMAN_REFUSAL_CONFIRM_SEC=0 FAKE_PANE_TEXT="nothing here" \
    FAKE_MARKER=timeout FAKE_STATUS=idle FOREMAN_NOW_EPOCH=1767225900 \
    bash "$SCRIPT" --once --worktree "$stall_wt" --base "$stall_base" \
    --since "2026-01-01T00:00:00+00:00" \
  ...
  RUN_SEQ=$((RUN_SEQ+1))
  OUT="$(env HERDR_ENV=1 HERDR_BIN="$FAKE" FOREMAN_WAIT_INTERVAL_SEC=0 FOREMAN_WAIT_BUDGET_SEC=0 \
    FOREMAN_BLOCKED_CONFIRM_SEC=0 FAKE_PANE_TEXT="REPORT: ${report}" \
