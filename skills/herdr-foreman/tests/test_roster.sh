#!/usr/bin/env bash
# Outcome-based tests for skills/herdr-foreman/roster.sh.
#
...
#
# Run: bash skills/herdr-foreman/tests/test_roster.sh
set -uo pipefail
...
main() {
  ...
  TMP="$(mktemp -d -t foreman-roster-test.XXXXXX)" || die "mktemp failed"
  trap cleanup EXIT
  ...
  # 6. No caller pane id -> refuse; the roster cannot exclude the foreman's pane.
