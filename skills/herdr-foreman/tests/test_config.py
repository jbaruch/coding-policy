...
from foreman.errors import ConfigError
...
class LoadConfigTest(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.mkdtemp(prefix="foreman-config-test-")
...
class DefaultPathTest(unittest.TestCase):
    ...
    def test_honours_xdg_config_home(self):
        ...
        self.assertEqual(default_config_path(), Path("/somewhere/cfg/foreman/config.json"))

    def test_falls_back_to_dot_config(self):
        os.environ.pop("XDG_CONFIG_HOME", None)
        self.assertEqual(
            default_config_path(), Path.home() / ".config" / "foreman" / "config.json"
        )
...
class LoadRoleCostsTest(unittest.TestCase):
    def setUp(self):
        self.tmp = Path(tempfile.mkdtemp(prefix="foreman-costs-test-"))
        self.addCleanup(shutil.rmtree, self.tmp)
