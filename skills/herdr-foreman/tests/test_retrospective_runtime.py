...
class RetrospectiveRuntimeTest(unittest.TestCase):
    ...
    def test_offline_retrieval_ignores_config_state_migrations_and_deleted_sources(self):
        ...
        for report in self.reports.values():
            report.unlink()
        before = self.path.read_bytes()
        with patch.dict(os.environ, {}, clear=True), patch("foreman.cli._client", side_effect=AssertionError("offline")), patch("foreman.cli.load_config", side_effect=AssertionError("offline")):
