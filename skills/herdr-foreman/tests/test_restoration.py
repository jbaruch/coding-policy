...
from foreman import cli, restoration
from foreman.errors import HerdrError, UsageError
from foreman.herdr import HerdrClient, error_code
from foreman.restoration import NAME_TAKEN_RETRIES, RELEASE_POLL_ATTEMPTS, RELEASE_POLL_INTERVAL_SEC, restore
...
class CliTest(RestorationCase):
    def setUp(self):
        super().setUp()
        self.tmp = Path(tempfile.mkdtemp(prefix="foreman-restore-test-"))
        ...
        for operation in ("pane process-info", "agent get"):
            for value in (None, [], "unexpected", 7, False):
                with self.subTest(operation=operation, value=value):
                    ...
                    out, err = io.StringIO(), io.StringIO()
                    with patch("foreman.cli.time.sleep", side_effect=self.sleep):
