#!/usr/bin/env bash
# Emit the Herdr worker roster the foreman skill dispatches a round to.
#
...
# pane is excluded so a foreman never dispatches a brief to itself.
#
# Contract:
#   argv  : none.
...
main() {
  ...
  if [[ -z "$caller" ]]; then
    warn "HERDR_PANE_ID is empty — Herdr injects it into every managed pane; re-run from the foreman's own pane"
    return 1
  fi
