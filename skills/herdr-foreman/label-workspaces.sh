#!/usr/bin/env bash
# Name the foreman's workspace and each worker's workspace and pane.
#
# A sidebar of `w1 w2 w3 w4` tells the operator nothing at 3am. Naming is one
# right answer per input — the foreman's workspace becomes <lead-label>, a
# worker's workspace becomes its agent name, and its pane becomes its kind —
# so it lives here rather than in the foreman's hands
...
main() {
  ...
  if [[ "${HERDR_ENV:-}" != "1" ]]; then
    warn "not running inside Herdr (HERDR_ENV='${HERDR_ENV:-}') — run this from the foreman's own pane"
    return 1
  fi
  local dep
  ...
  if [[ -z "$lead_ws" ]]; then
    warn "HERDR_WORKSPACE_ID is empty — Herdr injects it into every managed pane; re-run from the foreman's pane"
    return 1
  fi
