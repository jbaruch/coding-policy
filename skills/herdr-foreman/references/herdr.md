...
Everything the `herdr-foreman` skill needs to know about driving [Herdr](https://herdr.dev),
...
`skills/herdr-foreman/roster.sh` is the skill's read of this surface — see its
...
followed by verifying the file. `skills/herdr-foreman/wait-report.sh` does both;
...
marker list are constants at the top of `skills/herdr-foreman/wait-report.sh`.
...
Shipped defaults are in `skills/herdr-foreman/config.example.json`; the precedence
order and the row count are in `skills/herdr-foreman/foreman/probe.py`.
...
`--trace` (or `FOREMAN_TRACE=1`) prints every herdr invocation to stderr with
...
`skills/herdr-foreman/foreman/herdr.py`. It is signature matching, never proof: a
