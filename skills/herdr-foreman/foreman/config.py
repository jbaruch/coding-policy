...
The config is operator-owned. foreman never writes it -- the operator copies
...
def default_config_path():
    """`$XDG_CONFIG_HOME/foreman/config.json`, falling back to `~/.config`."""
    base = os.environ.get("XDG_CONFIG_HOME")
    root = Path(base) if base else Path.home() / ".config"
    return root / "foreman" / "config.json"
