...
The foreman stops a retained developer's own idle foreground process to restore
...
start with the foreman's already verified resume argv, retried only after a
name-reservation refusal and only once the pane and name re-prove released.

It chooses nothing. The foreman supplies the archived name, kind, pane, shell
...
and never writes owner state. Every judgment the reference keeps with the foreman
...
def register_commands(sub, common):
    parser = sub.add_parser(
        ...
        help="After the foreman stopped a retained worker's own process: wait for its pane shell and released name, then restart the same name with the resume argv given after --.",
    )
    ...
    parser.add_argument("--stopped-pid", required=True, type=int, metavar="PID", help="The foreground PID the foreman stopped.")
