...
Commands whose result foreman actually consumes are parsed strictly. Commands
foreman only fires and forgets go through `_run_optional_json`, which treats
...
guessing at what foreman sent. `--trace` or `FOREMAN_TRACE=1` turns it on.
...
#: Agent lifecycle states foreman refuses to write to. No override.
...
TRACE_ENV_VAR = "FOREMAN_TRACE"


def default_binary():
    """The herdr executable to invoke, overridable for an unusual install."""
    return os.environ.get("FOREMAN_HERDR_BIN", "herdr")


def trace_enabled_in_env(environ=None):
    """True when FOREMAN_TRACE is set to something other than 0 or empty."""
...
class HerdrClient:
    ...
    def _run(self, argv):
        ...
        try:
            ...
            raise HerdrError(
                ...
                "FOREMAN_HERDR_BIN to its absolute path.".format(self.binary),
    ...
    def _failure_message(argv, completed):
        ...
        if completed.returncode == 2:
            return (
                ...
                "build of foreman may not match the installed herdr; check "
    ...
    def _run_optional_json(self, argv):
        """Run a command whose result foreman never reads.
    ...
    def send_slash_command(self, pane_id, text, enter_count=1, before_input=None, after_submit=None):
        ...
        brief is a message, and pasting it is exactly right. `before_input`
        revalidates the caller's live guard before the text and each Enter.
        """
