...
its own reader; Claude's is foreman/claude_native.py. Unknown integrations,
        ...
        if kind == "codex":
            ...
            if row.get("type") == "response_item" and payload.get("role") == "user":
                ...
                if not isinstance(content, list) or not all(isinstance(item, dict) and item.get("type") == "input_text"
                        and isinstance(item.get("text"), str) for item in content):
                    return None
                # Codex 0.154.0 emits user-role runtime context after compaction.
                # Retain only a metadata-proven user.text prompt in that same
                # active turn. Unknown/mixed kinds refuse, even with matching
                ...
                if "internal_chat_message_metadata_passthrough" in payload:
                    ...
                    if kinds == ["environments.environment_context"]:
                        if prompt_turn != turn:
                            invalid_turn = True
                        continue
                    if kinds != ["user.text"]:
            ...
            if update.get("sessionUpdate") == "user_message_chunk":
                ...
                in_chunks = False
    return None if kind == "codex" and invalid_turn else prompt
