...
# With no OUTFILE, FOREMAN_REPORTS_DIR is required; the filename is
...
main() (
  ...
  if (( $# == 3 )); then
    outfile="$3"
  elif [[ -n "${FOREMAN_REPORTS_DIR:-}" ]]; then
    outfile="${FOREMAN_REPORTS_DIR}/review-${base:0:7}..${head:0:7}.diff"
  else
    review_package_warn 'set FOREMAN_REPORTS_DIR to the round reports directory, or pass OUTFILE there'
    return 2
  fi
