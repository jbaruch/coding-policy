...
- **plugin-evals** — Re-separate bleeding from tile-prescribed conformance. Earlier rule wording ("criteria may not reference string conventions the tile invented") was wrong and directly caused the v0.1.19 lift regression (1.35x → 1.15x, absolute lift halved): the tile's value IS prescribing specific conventions, and criteria checking those conventions measure tile application — they are NOT testing reading unless the convention literal also appears in the task. The corrected rule makes the distinction explicit: **task describes the situation** (no technique), **criteria check whether the output matches the specific manner the tile prescribes** (flag choices, format literals, sequences, chosen identifiers). Bleeding is strictly a criterion literal appearing in the task; fix it at the task, not by removing the criterion. Criteria may reference tile-prescribed conventions and invented literals — a competent engineer without the tile would not produce those specific choices, which is precisely why they measure tile value. Added a three-cause diagnosis for near-zero lift (coincidence with universal competence / task leak / criteria grading engineering-101) so "low lift = retire" is no longer the reflex.
