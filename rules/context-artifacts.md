...
- Wire `tessl skill review` into CI as a **changed-skills loop**, not as static per-skill steps. One workflow step drives the loop and iterates over the skills whose files changed since the previous push (`git diff --name-only <prev-sha>..HEAD -- 'skills/'` or equivalent (directory pathspec — matches what `action.yml` uses)); the loop reviews only those. `tessl skill review` is LLM-backed and credit-consuming, and running it against unchanged content reproduces the prior rubric output, so per-skill static steps are both costlier and lower-signal than the diff-driven loop. Reference implementation: `.github/actions/skill-review/action.yml` in this tile, consumed via `uses: jbaruch/coding-policy/.github/actions/skill-review@<ref>` from consumer repos. Adding a skill therefore requires no CI YAML edit — the loop picks the new path up automatically the first time it appears in the diff. The loop reviews every skill only when the diff base is absent (manual `workflow_dispatch`, initial push, the all-zeros sentinel SHA); when the diff base is set but unreachable in the local clone the loop hard-fails rather than silently degrading to either "review all" or "no changes" — both would mask a missing review under the changed-only contract
