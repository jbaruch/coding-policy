...
- Optional fields include `allowed-tools`, `disable-model-invocation`, `user-invocable` (set to `false` for background-knowledge skills the runtime loads as context but the user should never invoke directly), and `argument-hint` (Claude Code slash-command UI hint — a string naming expected arguments, e.g. `[init|status|use|uninit|help] [feature]`)
