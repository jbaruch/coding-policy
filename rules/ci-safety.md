...
- Applies when a gating bot that cannot `APPROVE` (`chatgpt-codex-connector[bot]` posts only `CHANGES_REQUESTED` or `COMMENT`, never `APPROVE`) re-reviews clean but cannot post the `APPROVED` verdict that would supersede its earlier `CHANGES_REQUESTED` — the stale request keeps the merge `BLOCKED` until dismissed
